...
namespace survey {
QuizChoiceBlock::QuizChoiceBlock(const nlohmann::json &block, QWidget *parent)
...
void QuizChoiceBlock::save_answer(nlohmann::json &answer_data) const {
    answer_data.push_back({{"answer", options_->checkedId()}});
}

bool QuizChoiceBlock::has_answer() const {
    return options_->checkedId() != -1;
}

void QuizChoiceBlock::set_answer(const nlohmann::json &answer) {
...
void QuizChoiceBlock::set_read_only(bool read_only) {
